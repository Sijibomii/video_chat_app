...
interface createBroad{
   id: String, displayName: String, device : {
...
interface createBroadProd{
    broadcasterId : String
    transportId: String
    kind: 'audio' | 'video'
    rtpParameters :any //for now //RTP parameters for the Producer.
}
interface consumBroad{
  broadcasterId: String
  transportId : String
  producerId: String
}
...
interface createCon{ consumerPeer:any, producerPeer:any, producer:any }
interface createDataCon	{
  dataConsumerPeer: any
  dataProducerPeer : any | null // This is null for the bot DataProducer.
  dataProducer:any
}
class Room extends EventEmitter {
  ...
  logStatus(){
	 ...
	 * browser.
	 *
	 *  peerId - The id of the protoo peer to be created.
	 *  consume - Whether this peer wants to consume from others.
	 * protooWebSocketTransport - The associated protoo WebSocket transport.
	 */
	handleProtooConnection(prooto: prooto){
  ...
  /**
	 ...
	 */
	async createBroadcaster(createBroad: createBroad){
  ...
  /**
	 ...
	 */
	async createBroadcasterTransport(createBroad:createBroad){
    const {
			broadcasterId,
			type,
			rtcpMux = false,
  ...
  _handleAudioLevelObserver()
	 ...
	 * @async
	 */
	async _handleProtooRequest(peer:any, request:any, accept:any, reject:any)
	{
  ...
  /**
	...
	_getJoinedPeers({ excludePeer = undefined } = {})
	{
		return this._protooRoom.peers
			.filter((peer:any) => peer.data.joined && peer !== excludePeer);
	}
  /**
	...
	async _createConsumer(createCon: createCon){
	 ...
	 * @async
	 */
	async _createDataConsumer(createDataCon:createDataCon){
